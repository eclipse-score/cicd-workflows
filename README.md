...
covered by `on-pr.yml`, `docs.yml`, or `daily.yml`. The documentation cleanup
implementation remains available as an internal helper for `daily.yml`; external
callers should migrate to the replacement workflow.
...
| `docs-cleanup.yml`       | `daily.yml`                    | Renamed to `_docs-cleanup.yml` and kept as an internal helper invoked by `daily.yml`.                     |
